...
const access_Token="pk.eyJ1Ijoia2FuZWtpcml0bzEyNzciLCJhIjoiY2t1MGphMm4wMHV4eDJvczhzMmp3Z2dwNyJ9.lXjaIHaVdGyfuKN_aXZI_g";
...
const map = new mapboxgl.Map({
  ...
  bounds:[[102.144796000004,8.3141966009692],[109.5694718,23.3922699957244]],
});
  ...
  map.on(trangthai, () => {
    ...
    map.on("click", "states-layer", (e) => {
      if (click == 0) {
        ...
        fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${ten_Tinh}.json?access_token=${access_Token}`)
        .then(response=>response.json())
        .then(data=>{
          let bbox;
          let i=0;
      ...
      if (click == 1) {
        ...
        if (e.features[0].properties.NAME_1=="Hà Nội")
          ten_Tinh="Hanoi Vietnam";
        let tenHuyen=document.querySelector("h3").textContent+" "+ten_Tinh;
        console.log(tenHuyen);
        fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${tenHuyen}.json?access_token=${access_Token}`)
        .then(response=>response.json())
        .then(data=>{
          let bbox;
          let i=0;
      ...
      if (click == 2) {
        ...
        fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${tenXaDayDu}.json?access_token=${access_Token}`)
        .then(response=>response.json())
        .then(data=>{
          let bbox;
          let i=0;
